...
void insert_window(BSPNode **root, Window w) {
    ...
    // Find a leaf to split (just get any leaf for now)
    BSPNode *target = get_any_leaf(*root);
    if (!target || !target->is_leaf) return;
    
    // Alternate split direction based on current count
    SplitType split = (count_leaves(*root) % 2 == 0) ? SPLIT_VERTICAL : SPLIT_HORIZONTAL;
